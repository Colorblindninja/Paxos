...
func (command Command) String() string {
	return fmt.Sprintf("Command:\nTag: %d\n %s\n %s\nAddress: %s", command.Tag, command.Command, command.Sequence.String(), command.Address)
}
...
func (slot Slot) String() string {
	return fmt.Sprintf("Slot:\nCommand: %v\nDecided: %t", slot.Command, slot.Decided)
}
...
func (self *Replica) Prepare(req Request, resp *Response) error {
	...
	args := req.Prepare
	logM("")
	logM("Prepare:  Slot: " + strconv.Itoa(args.Slot.N) + ", Sequence: " + strconv.Itoa(args.Seq.N))
	...
	if self.Slots[args.Slot.N].Sequence.Cmp(args.Seq) == -1 {
		...
		resp.Okay = false
		logM("Prepare:  Didn't work. Already promised a higher number: " + strconv.Itoa(self.Slots[args.Slot.N].Sequence.N))
...
func (self *Replica) Accepted(req Request, resp *Response) error {
	...
	args := req.Accepted
	logM("")
	logM("Accepted:  Slot: " + strconv.Itoa(args.Slot.N) + ", Sequence " + strconv.Itoa(args.Seq.N))
	logM("Accepted:  " + args.Slot.String())

	if self.Slots[args.Slot.N].Sequence.Cmp(args.Seq) == 0 {
		logM("Accepted: Worked")
		resp.Okay = true
		resp.Promised = self.Slots[args.Slot.N].Sequence
		self.Recent = args.Slot
	} else {
		logM("Accepted:  Didn't work. Already promised a higher number: " + strconv.Itoa(self.Slots[args.Slot.N].Sequence.N))
		resp.Okay = false
		//resp.Promised = self.Slots[args.Slot.N].Sequence
...
func (self *Replica) Decide(req Request, resp *Response) error {
	...
	args := req.Decide
	logM("")
	logM("Decide:  (SLOT: " + strconv.Itoa(args.Slot.N) + ", VALUE: " + args.Value.Command + ")")
	...
	case "put":
		...
		break
	case "get":
		log.Println("Get: " + command[1] + ", " + self.Database[command[1]])
		break
	case "delete":
		log.Println(command[1] + " was deleted.")
...
func (self *Replica) Propose(req Request, resp *Response) error {
	...
	args := req.Propose
	logM("")
	logM("Propose: " + args.Command.String())
...
func (self *Replica) Accept(req Request, resp *Response) error {
	...
	logM("")
	logM("Accept:  Command: " + aV.String() + ", Sequence: " + aN.String() + ", Slot: " + aSlot.String())
	...
	if numYes >= majority(len(self.Friends)) {
		logM("Accept Received enough votes.")
		for _, v := range self.Friends {
...
func help() {
	fmt.Println("help displays possible options")
	fmt.Println("dump shows info about current node")
